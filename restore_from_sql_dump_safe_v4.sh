#!/bin/bash
# Safely restore UGC tables from SQL text dump file (v4 - awk quoting fix)
...
convert_copy_to_insert() {
    ...
    # Corrected awk script - v4 using variable for quotes
    tail -n +2 "$input_file" | head -n -1 | awk -F'\t' -v table="$table_name" -v cols="$columns" -v q="'" '
    {
        if ($0 == "\\.") next; 
        ...
        for (i = 1; i <= NF; i++) {
            if ($i == "\\N") {
                ...
                if ($i ~ /^[+-]?[0-9]+(\.[0-9]+)?([eE][+-]?[0-9]+)?$/ || $i ~ /^(true|false|TRUE|FALSE)$/) {
                    ...
                    printf "%s%s%s", q, $i, q; # Enclose in single quotes using the q variable
                }
            }
            if (i < NF) printf ", ";
        }
