...
const PROVIDER_CONFIGS = {
    github: {
        name: 'GitHub',
        icon: (<svg viewBox="0 0 20 20" className="w-5 h-5" fill="currentColor">
...
function getEnabledProviders() {
    ...
    return providersStr.split(',').filter((provider) => provider.trim().toLowerCase() in PROVIDER_CONFIGS);
}
export default function SSOButtons({ onError }) {
    const handleSSOLogin = async (provider) => {
        try {
            ...
            if (err instanceof Error) {
                ...
                onError === null || onError === void 0 ? void 0 : onError('An unknown error occurred');
            }
        }
    };
    ...
    return (<div className="mt-6">
            ...
            return (<button key={provider} onClick={() => handleSSOLogin(provider)} className={`group relative flex h-11 items-center rounded-md border ${config.borderColor} px-6 transition-colors duration-200 ${config.bgColor} ${config.textColor}`}>
