...
export default function PowerBriefPage() {
    ...
    const handleCreateBrand = async () => {
        if (!(user === null || user === void 0 ? void 0 : user.id) || !newBrandName.trim())
            return;
        try {
            setCreatingBrand(true);
            // Create empty data structures for new brand
            const emptyBrandInfo = {
                positioning: '',
                product: '',
                technology: '',
                testimonials: '',
    ...
    return (<div className="space-y-6 p-6">
            <div className="flex justify-between items-center">
                ...
                <Button className="bg-primary-600 text-white hover:bg-primary-700" onClick={() => setShowNewBrandDialog(true)}>
            ...
            {brands.length === 0 ? (<Card>
                    <CardContent className="p-8 text-center text-gray-500">
                        ...
                        <Button className="mt-6 bg-primary-600 text-white hover:bg-primary-700" onClick={() => setShowNewBrandDialog(true)}>
                            <Plus className="h-4 w-4 mr-2"/>
                            Add New Brand
                        </Button>
                    </CardContent>
                </Card>) : (<div className="flex overflow-x-auto pb-4 space-x-6 hide-scrollbar">
                    {brands.map((brand) => (<Link href={`/app/powerbrief/${brand.id}`} key={brand.id} className="min-w-[300px]">
            ...
            <Dialog open={showNewBrandDialog} onOpenChange={setShowNewBrandDialog}>
                <DialogContent>
                    ...
                    <div className="py-4">
                        <Input placeholder="Brand Name" value={newBrandName} onChange={(e) => setNewBrandName(e.target.value)} disabled={creatingBrand}/>
                    </div>
                    
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setShowNewBrandDialog(false)} disabled={creatingBrand}>
                            Cancel
                        </Button>
                        <Button className="bg-primary-600 text-white hover:bg-primary-700" onClick={handleCreateBrand} disabled={!newBrandName.trim() || creatingBrand}>
                            {creatingBrand ? (<>
