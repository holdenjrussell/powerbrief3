...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    for (const adDraft of adDrafts) {
      try {
        console.log(`Processing ad draft: ${adDraft.id} - ${adDraft.ad_name}`);
        ...
        const systemPrompt = `You are an AI assistant that generates professional ad names based on video content and a structured naming convention.
...
${namingConventionContext.prefixes
  ...
  .map(prefix => {
    if (prefix.meaning === 'Launch Date') {
      ...
      return `- ${prefix.prefix}: Extract ${prefix.meaning} from video content`;
    }
  }).join('\n')}
...
${namingConventionContext.prefixes
  ...
  .map(prefix => {
    const abbreviationsList = prefix.abbreviations.map(a => `${a.key} (${a.full_name}: ${a.description})`).join(', ');
    return `- ${prefix.prefix}: Analyze video content and select the most appropriate abbreviation from: ${abbreviationsList}`;
  }).join('\n')}
...
${namingConventionContext.prefixes
  ...
  .map(prefix => {
    return `- ${prefix.prefix}: Generate a unique 4-5 word plain English description of the video content that captures the main concept, style, or messaging (e.g., "energetic workout motivation video", "calm skincare routine demo", "funny product comparison skit")`;
  }).join('\n')}
...
INSTRUCTIONS:
1. Analyze the video content to understand the creative concept, style, and messaging
...
4. For abbreviation prefixes, select the most appropriate abbreviation based on video analysis
5. For wildcard prefixes, generate a unique 4-5 word plain English description that captures the essence of the video
...
Generate a single, professional ad name that follows this exact structure.`;

        const userPrompt = `Please generate a new ad name for this video content:
...
Based on the video content and the naming convention structure provided, generate a single, optimized ad name following the exact prefix order and format specified.`;
          ...
          if (firstAsset.supabase_url && (firstAsset.type === 'video' || firstAsset.type === 'image')) {
            try {
              ...
              if (assetResponse.ok) {
                ...
                console.log(`Adding asset to request: ${assetBlob.size} bytes, mime type: ${mimeType}`);
    ...
    console.log(`Bulk rename completed. ${results.filter(r => r.success).length}/${results.length} ads renamed successfully`);
