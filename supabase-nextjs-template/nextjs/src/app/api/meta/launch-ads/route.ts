    ...
    for (const draft of drafts) {
      ...
      for (const asset of draft.assets) {
        ...
        for (let attempt = 0; attempt < maxRetries && !uploadSuccess; attempt++) {
          try {
            if (attempt > 0) {
              console.log(`[Launch API]     Retry attempt ${attempt + 1}/${maxRetries} for ${asset.name}`);
              // Wait before retry (exponential backoff)
              await new Promise(resolve => setTimeout(resolve, Math.pow(2, attempt) * 1000));
            }
            
            // Add more detailed logging for the specific problematic file
            if (asset.name.includes('Concept-55 - HO3 - Maarij - Holden - 4x5.mp4')) {
              console.log(`[Launch API]     *** DEBUGGING PROBLEMATIC FILE: ${asset.name} ***`);
              console.log(`[Launch API]     Asset details:`, {
                name: asset.name,
                type: asset.type,
            ...
            // Log blob details for problematic file
            if (asset.name.includes('Concept-55 - HO3 - Maarij - Holden - 4x5.mp4')) {
              console.log(`[Launch API]     Blob details:`, {
                size: assetBlob.size,
                type: assetBlob.type
              });
            }
            ...
            if (asset.type === 'video') {
              // Check file size (Meta limit is typically 4GB, but let's warn for large files)
              const maxSizeBytes = 4 * 1024 * 1024 * 1024; // 4GB
              if (assetBlob.size > maxSizeBytes) {
                console.error(`[Launch API]     Video ${asset.name} is too large: ${assetBlob.size} bytes (max: ${maxSizeBytes})`);
                updatedAssets.push({ ...asset, metaUploadError: `Video file too large: ${Math.round(assetBlob.size / (1024 * 1024))}MB (max: 4GB)` });
                continue;
              }
              
              // Check if it's a supported video format
              const supportedVideoTypes = ['video/mp4', 'video/mov', 'video/avi', 'video/quicktime'];
              if (!supportedVideoTypes.includes(assetBlob.type) && !assetBlob.type.startsWith('video/')) {
                console.warn(`[Launch API]     Video ${asset.name} has unsupported MIME type: ${assetBlob.type}`);
                // Don't fail here, let Meta decide if it's acceptable
              }
              
              console.log(`[Launch API]     Video validation passed for ${asset.name}: ${Math.round(assetBlob.size / (1024 * 1024))}MB, type: ${assetBlob.type}`);
            }
            ...
            const metaResponse = await fetch(metaUploadUrl, {
              method: 'POST',
              body: formData,
              // Add timeout for video uploads (they can take longer)
              signal: AbortSignal.timeout(asset.type === 'video' ? 300000 : 60000), // 5 minutes for video, 1 minute for images
            });

            // Check if response has content before trying to parse JSON
            const responseText = await metaResponse.text();
            console.log(`[Launch API]     Meta response status: ${metaResponse.status}, body length: ${responseText.length}`);
            
            let metaResult;
            try {
              if (responseText.trim() === '') {
                console.error(`[Launch API]     Empty response from Meta for ${asset.name}`);
                updatedAssets.push({ ...asset, metaUploadError: 'Empty response from Meta API' });
                continue;
              }
              metaResult = JSON.parse(responseText);
            } catch (jsonError) {
              console.error(`[Launch API]     Failed to parse JSON response for ${asset.name}:`, jsonError);
              console.error(`[Launch API]     Response text:`, responseText);
              updatedAssets.push({ ...asset, metaUploadError: `Invalid JSON response from Meta: ${responseText.substring(0, 200)}` });
              continue;
            }
            ...
            if (uploadError instanceof Error) {
              if (uploadError.name === 'AbortError') {
                errorMessage = `Upload timeout - ${asset.type} upload took too long`;
              } else if (uploadError.message.includes('fetch')) {
                errorMessage = `Network error during upload: ${uploadError.message}`;
              } else if (uploadError.message.includes('JSON')) {
                errorMessage = `Invalid response from Meta API: ${uploadError.message}`;
              }
            }
            
            updatedAssets.push({ ...asset, metaUploadError: errorMessage });
            lastError = uploadError as Error;
          }
        }
        
        // If all retries failed, add the asset with the last error
        if (!uploadSuccess && lastError) {
          console.error(`[Launch API]     All retry attempts failed for ${asset.name}:`, lastError.message);
          updatedAssets.push({ ...asset, metaUploadError: lastError.message });
        }
      }
