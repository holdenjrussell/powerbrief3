-- Add comprehensive dashboard tables for payments, contracts, and shipments
-- This migration extends the UGC workflow system with financial and operational tracking
-- Fixed version with proper table creation order

-- Monthly Budget Tracking
CREATE TABLE IF NOT EXISTS public.ugc_monthly_budgets (
...
CREATE TABLE IF NOT EXISTS public.ugc_payments (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  brand_id UUID NOT NULL REFERENCES public.brands(id) ON DELETE CASCADE,
...
-- Contract Templates (create this first, before contracts table)
CREATE TABLE IF NOT EXISTS public.ugc_contract_templates (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  brand_id UUID NOT NULL REFERENCES public.brands(id) ON DELETE CASCADE,
  name TEXT NOT NULL,
  description TEXT,
...
-- Contract Management (now references the templates table created above)
CREATE TABLE IF NOT EXISTS public.ugc_contracts (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  brand_id UUID NOT NULL REFERENCES public.brands(id) ON DELETE CASCADE,
  creator_id UUID NOT NULL REFERENCES public.ugc_creators(id) ON DELETE CASCADE,
...
-- Shipment Management
CREATE TABLE IF NOT EXISTS public.ugc_shipments (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  brand_id UUID NOT NULL REFERENCES public.brands(id) ON DELETE CASCADE,
  creator_id UUID NOT NULL REFERENCES public.ugc_creators(id) ON DELETE CASCADE,
  workflow_execution_id UUID REFERENCES public.ugc_workflow_executions(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS public.ugc_shipment_history (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
-- Add payment tracking fields to existing scripts table
...
ADD COLUMN IF NOT EXISTS shipment_required BOOLEAN DEFAULT true,
ADD COLUMN IF NOT EXISTS shipment_id UUID;

-- Add foreign key constraints after tables are created
ALTER TABLE public.ugc_creator_scripts 
ADD CONSTRAINT fk_ugc_creator_scripts_contract_id 
FOREIGN KEY (contract_id) REFERENCES public.ugc_contracts(id) ON DELETE SET NULL;

ALTER TABLE public.ugc_creator_scripts 
ADD CONSTRAINT fk_ugc_creator_scripts_shipment_id 
FOREIGN KEY (shipment_id) REFERENCES public.ugc_shipments(id) ON DELETE SET NULL;

-- Add contract and shipment tracking to creators
...
-- Add foreign key constraint for creators table
ALTER TABLE public.ugc_creators
ADD CONSTRAINT fk_ugc_creators_contract_template_id 
FOREIGN KEY (contract_template_id) REFERENCES public.ugc_contract_templates(id) ON DELETE SET NULL;

-- Indexes for performance
CREATE INDEX IF NOT EXISTS idx_ugc_monthly_budgets_brand_month ON public.ugc_monthly_budgets(brand_id, month_year);
CREATE INDEX IF NOT EXISTS idx_ugc_payments_brand_id ON public.ugc_payments(brand_id);
CREATE INDEX IF NOT EXISTS idx_ugc_payments_creator_id ON public.ugc_payments(creator_id);
CREATE INDEX IF NOT EXISTS idx_ugc_payments_status ON public.ugc_payments(status);
CREATE INDEX IF NOT EXISTS idx_ugc_payments_due_date ON public.ugc_payments(due_date);
CREATE INDEX IF NOT EXISTS idx_ugc_contracts_brand_id ON public.ugc_contracts(brand_id);
CREATE INDEX IF NOT EXISTS idx_ugc_contracts_creator_id ON public.ugc_contracts(creator_id);
CREATE INDEX IF NOT EXISTS idx_ugc_contracts_status ON public.ugc_contracts(status);
CREATE INDEX IF NOT EXISTS idx_ugc_contract_templates_brand_id ON public.ugc_contract_templates(brand_id);
CREATE INDEX IF NOT EXISTS idx_ugc_shipments_brand_id ON public.ugc_shipments(brand_id);
CREATE INDEX IF NOT EXISTS idx_ugc_shipments_creator_id ON public.ugc_shipments(creator_id);
CREATE INDEX IF NOT EXISTS idx_ugc_shipments_status ON public.ugc_shipments(status);
...
-- RLS Policies
CREATE POLICY "Users can manage budgets for their brands" ON public.ugc_monthly_budgets
  FOR ALL USING (
    brand_id IN (
...
CREATE POLICY "Users can manage payments for their brands" ON public.ugc_payments
  FOR ALL USING (
    brand_id IN (
      SELECT brand_id FROM public.user_brand_access 
      WHERE user_id = auth.uid()
...
CREATE POLICY "Users can manage contracts for their brands" ON public.ugc_contracts
  FOR ALL USING (
    brand_id IN (
      SELECT brand_id FROM public.user_brand_access 
      WHERE user_id = auth.uid()
...
CREATE POLICY "Users can manage contract templates for their brands" ON public.ugc_contract_templates
  FOR ALL USING (
    brand_id IN (
      SELECT brand_id FROM public.user_brand_access 
      WHERE user_id = auth.uid()
...
CREATE POLICY "Users can manage shipments for their brands" ON public.ugc_shipments
  FOR ALL USING (
    brand_id IN (
      SELECT brand_id FROM public.user_brand_access 
      WHERE user_id = auth.uid()
...
CREATE POLICY "Users can view shipment history for their brands" ON public.ugc_shipment_history
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.ugc_shipments s
      WHERE s.id = ugc_shipment_history.shipment_id
      AND s.brand_id IN (
        SELECT brand_id FROM public.user_brand_access 
        WHERE user_id = auth.uid()
      )
...
CREATE TRIGGER update_ugc_monthly_budgets_updated_at 
  BEFORE UPDATE ON public.ugc_monthly_budgets 
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

CREATE TRIGGER update_ugc_payments_updated_at 
  BEFORE UPDATE ON public.ugc_payments 
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

CREATE TRIGGER update_ugc_contracts_updated_at 
  BEFORE UPDATE ON public.ugc_contracts 
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

CREATE TRIGGER update_ugc_contract_templates_updated_at 
  BEFORE UPDATE ON public.ugc_contract_templates 
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

CREATE TRIGGER update_ugc_shipments_updated_at 
...
DECLARE
  target_month DATE;
BEGIN
  -- Get the first day of the month for the payment
...
RETURNS TRIGGER AS $$
BEGIN
  -- Only update budget when payment status changes to 'paid'
  IF NEW.status = 'paid' AND (OLD.status IS NULL OR OLD.status != 'paid') THEN
    PERFORM update_monthly_budget_spent(NEW.brand_id, NEW.amount, NEW.paid_date);
  END IF;
  
  -- If payment is cancelled or failed, subtract from budget
