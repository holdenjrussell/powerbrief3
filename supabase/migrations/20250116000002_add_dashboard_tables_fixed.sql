-- UGC Dashboard Tables Migration
...
-- RLS Policies using correct brand_shares table
CREATE POLICY "Users can manage budgets for accessible brands" ON public.ugc_monthly_budgets
  FOR ALL USING (
    brand_id IN (
...
CREATE POLICY "Users can manage payments for accessible brands" ON public.ugc_payments
  FOR ALL USING (
    brand_id IN (
      SELECT id FROM public.brands WHERE user_id = auth.uid()
      UNION
...
CREATE POLICY "Users can manage contracts for accessible brands" ON public.ugc_contracts
  FOR ALL USING (
    brand_id IN (
      SELECT id FROM public.brands WHERE user_id = auth.uid()
      UNION
...
CREATE POLICY "Users can manage contract templates for accessible brands" ON public.ugc_contract_templates
  FOR ALL USING (
    brand_id IN (
      SELECT id FROM public.brands WHERE user_id = auth.uid()
      UNION
...
CREATE POLICY "Users can manage shipments for accessible brands" ON public.ugc_shipments
  FOR ALL USING (
    brand_id IN (
      SELECT id FROM public.brands WHERE user_id = auth.uid()
      UNION
...
CREATE POLICY "Users can view shipment history for accessible brands" ON public.ugc_shipment_history
  FOR ALL USING (
    EXISTS (
