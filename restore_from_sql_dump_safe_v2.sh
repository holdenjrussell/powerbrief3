...
convert_copy_to_insert() {
    ...
    # This regex handles cases where column names might have spaces or be quoted
    local columns_line=$(head -1 "$input_file")
    local columns=$(echo "$columns_line" | sed -E 's/COPY public\."?'"$table_name"'"? \((.*)\) FROM stdin;/\1/')
    ...
    {
        if ($0 == "\\.") next; # Skip the final \. line
        
        printf "INSERT INTO public.%s (%s) VALUES (", table, cols);
        for (i = 1; i <= NF; i++) {
            # Handle NULL values (represented as \N in COPY FROM stdin)
            if ($i == "\\N") {
                printf "NULL";
            } else {
                # Escape single quotes within the data
                gsub(/'/,"'''", $i);
                # Properly quote string literals, handle numeric and boolean values
                # This is a simplified check; for complex types, direct COPY is often better
                # or a more sophisticated parser is needed.
                # For now, we assume text-like fields need quoting.
                # A more robust solution for various data types would involve checking schema or using a language like Python.
                if ($i ~ /^[0-9]+(\.[0-9]+)?$/ || $i ~ /^(true|false)$/i) {
                    printf "%s", $i;
                } else {
                    printf "'%s'", $i;
                }
